...
import com.microb.auth.security.JWTAuthenticationFilter
...
class JWTService(
    ...
    fun createToken(username: String, password: String): String {
        ...
        return Jwts.builder()
                ...
                .setIssuedAt(Date())
                .setExpiration(Date(System.currentTimeMillis() + JWTAuthenticationFilter.EXPIRATION_TIME))
                .signWith(SignatureAlgorithm.HS512, JWTAuthenticationFilter.SECRET.toByteArray())
                .compact()
    }

    fun getTokenIfValid(token: String): Jws<Claims>? {
        return try {
            Jwts.parser().setSigningKey(JWTAuthenticationFilter.SECRET.toByteArray()).parseClaimsJws(token)
        } catch (e: Exception) {
            when (e) {
